...
(defn- normalize-validator-stats-window
  [window]
  (when (map? window)
    {:uptime-fraction (common/optional-number (or (:uptimeFraction window)
                                           (:uptime-fraction window)))
     :predicted-apr (common/optional-number (or (:predictedApr window)
                                         (:predicted-apr window)))
     :sample-count (some common/parse-ms
                         [(:nSamples window)
                          (:sampleCount window)
                          (:sample-count window)])}))
...
(defn- normalize-validator-stake
  [value]
  (when-let [stake (common/optional-number value)]
    (if (> stake 1000000000)
...
(defn- normalize-validator-summary-row
  [row]
  (when (map? row)
    (let [validator (common/normalize-address (:validator row))
          signer (common/normalize-address (:signer row))
          stats (:stats row)]
      (when (seq validator)
         {:validator validator
          :signer signer
         ...
         :commission (common/optional-number (:commission row))
...
(defn- normalize-delegator-summary
  [payload]
  (if (map? payload)
    {:delegated (common/optional-number (:delegated payload))
     :undelegated (common/optional-number (:undelegated payload))
     :total-pending-withdrawal (common/optional-number (or (:totalPendingWithdrawal payload)
                                                    (:total-pending-withdrawal payload)))
     :pending-withdrawals (some common/parse-ms
...
(defn- normalize-delegation-row
  ...
  (when (map? row)
    (let [validator (common/normalize-address (:validator row))]
      (when (seq validator)
        {:validator validator
         :amount (common/optional-number (:amount row))
         :locked-until-timestamp (some common/parse-ms
...
(defn- normalize-delegator-reward-row
  [row]
  (when (map? row)
    {:time-ms (some common/parse-ms [(:time row) (:time-ms row)])
     :source (keyword (str (or (:source row) "unknown")))
     :total-amount (common/optional-number (or (:totalAmount row)
                                        (:total-amount row)
                                        (:amount row)))}))
...
(defn- normalize-delegate-history-delta
  ...
  (let [undelegate? (true? (:isUndelegate delegate))]
    {:kind (if undelegate?
             :undelegate
             :delegate)
     :validator (common/normalize-address (:validator delegate))
     :amount (common/optional-number (:amount delegate))
     :is-undelegate? undelegate?}))

(defn- normalize-c-deposit-history-delta
  [c-deposit]
  {:kind :deposit
   :amount (common/optional-number (:amount c-deposit))})

(defn- normalize-c-withdraw-history-delta
  [c-withdraw]
  {:kind :withdraw
   :amount (common/optional-number (:amount c-withdraw))})

(defn- normalize-withdrawal-history-delta
  [withdrawal]
  {:kind :withdrawal
   :amount (common/optional-number (:amount withdrawal))
...
(defn- normalize-delegator-history-row
  [row]
  (when (map? row)
    (let [time-ms (some common/parse-ms [(:time row) (:time-ms row)])]
      (when (number? time-ms)
        {:time-ms time-ms
