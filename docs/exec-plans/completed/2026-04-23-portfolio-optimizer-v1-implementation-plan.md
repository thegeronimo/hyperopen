...
- [x] (2026-04-23) Added Phase 4 split-variable gross-exposure expansion for the package adapters. Signed weights are expanded into positive/negative solver variables for `sum(abs(w)) <= gross-max`, original bounds and net/return constraints are projected into split-variable space, and solver output is decoded back to signed weights.
...
- [x] (2026-04-23) Added the first Phase 6 setup foundation: explicit optimizer defaults for draft, run-state, scenario index, and route-owned UI state; app defaults now initialize `[:portfolio :optimizer]` and `[:portfolio-ui :optimizer]`; the setup workspace renders separate objective, return-model, risk-model, and mandatory constraints panels from draft state.
- [x] (2026-04-23) Ran focused Phase 6 defaults/workspace validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.defaults-test --test=hyperopen.state.app-defaults-test --test=hyperopen.views.portfolio.optimize.view-test`; 13 tests and 203 assertions passed with zero failures and zero warnings. Added and ran a focused browser regression with `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "portfolio optimizer setup exposes separate model layers"`; 1 test passed.
- [x] (2026-04-23) Added the second Phase 6 setup slice: runtime-registered draft mutation actions for objective, return model, risk model, and supported constraints; setup model-layer chips are now clickable and mark the draft dirty through the app registry.
- [x] (2026-04-23) Ran focused draft-control validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.portfolio.optimizer.defaults-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.views.portfolio.optimize.view-test`; 26 tests and 200 assertions passed with zero failures and zero warnings. Recompiled the portfolio target and reran the focused browser regression; 1 Playwright test passed.
- [x] (2026-04-23) Added editable Phase 6 global constraint controls for long-only, max asset weight, gross leverage, net exposure bounds, dust threshold, max turnover, and rebalance tolerance; each control dispatches the runtime-registered constraint mutation action and preserves the read-only allowlist/blocklist and per-perp override summaries for later slices.
- [x] (2026-04-23) Ran focused global-constraint control validation with `npm run test:runner:generate`, `npx shadow-cljs --force-spawn compile test`, and `node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 3 tests and 31 assertions passed with zero failures and zero warnings. Recompiled the app target and reran `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "portfolio optimizer setup exposes separate model layers"`; 1 Playwright test passed.
- [x] (2026-04-23) Added a Phase 6 universe setup panel and runtime action to seed the optimizer draft universe from the existing current-portfolio abstraction, preserving instrument id, market type, coin, dex, and spot/perp shortability semantics while marking the draft dirty.
- [x] (2026-04-23) Ran focused universe setup validation with `npm run test:runner:generate`, `npx shadow-cljs --force-spawn compile test`, and `node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.views.portfolio.optimize.view-test`; 25 tests and 197 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 run-launch slice: the setup route now exposes a disabled-until-universe run control, and the runtime action builds a normalized optimizer request from the mutable draft, current portfolio snapshot, history cache, BL market-cap cache, and runtime freshness settings before dispatching the existing worker run effect with a request signature.
- [x] (2026-04-24) Ran focused run-launch validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.views.portfolio.optimize.view-test`; 28 tests and 213 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 setup-readiness slice: setup run gating now uses the assembled optimizer request, not just draft universe presence, so missing or insufficient history keeps the worker run blocked while preserving request-builder warnings in the right rail.
- [x] (2026-04-24) Ran focused readiness validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.views.portfolio.optimize.view-test`; 14 tests and 62 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 request-contract correction: draft-facing setup constraint keys now normalize into engine/domain keys, empty allowlists no longer filter out the universe, worker-decoded vector filters are accepted by the domain, and `:dust-usdc` is converted to a weight threshold from NAV before weight cleaning.
- [x] (2026-04-24) Ran focused constraint-contract validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.request-builder-test --test=hyperopen.portfolio.optimizer.application.engine-test --test=hyperopen.portfolio.optimizer.domain.constraints-test`; 15 tests and 69 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Added a worker-boundary serialization correction: worker-decoded instrument-id keyed maps are normalized back to string keys before entering the engine, covering current weights, return/funding history, BL prior weights, constraint overrides, and execution-assumption maps.
- [x] (2026-04-24) Ran focused worker-boundary validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.worker-test --test=hyperopen.portfolio.optimizer.application.run-bridge-test`; 8 tests and 25 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Added a Phase 6 objective/execution-assumptions setup slice: target-return and target-volatility inputs now dispatch draft mutations, execution assumptions are editable from setup, and fallback slippage normalizes into the engine request contract.
- [x] (2026-04-24) Ran focused objective/execution-assumptions validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.portfolio.optimizer.application.request-builder-test --test=hyperopen.views.portfolio.optimize.view-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test`; 37 tests and 266 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 row-level setup-controls slice: per-instrument allowlist, blocklist, max weight override, held-position lock, and per-perp cap controls now dispatch runtime-registered draft mutations from the setup workspace.
- [x] (2026-04-24) Ran focused row-level setup validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.views.portfolio.optimize.view-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test`; 34 tests and 257 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 setup run-state visibility slice: the left rail now shows draft dirty state, the run button reflects in-flight worker status, and the right rail shows run status, failed-run details, and retained last-successful-run context during reruns.
- [x] (2026-04-24) Ran focused run-state UI validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 7 tests and 66 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added a Phase 6 history-load slice: setup now exposes a load-history action, runtime effect, default load state, and right-rail loading/error/success messaging that fetches arbitrary-universe candle and funding history into optimizer-owned state without relying on the active-asset candle path.
- [x] (2026-04-24) Ran focused history-load validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.portfolio.optimizer.defaults-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.runtime.effect-adapters.facade-contract-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test --test=hyperopen.views.portfolio.optimize.view-test`; 63 tests and 388 assertions passed with zero failures and zero warnings. Recompiled the app target and reran the focused portfolio optimizer Playwright regression; 1 test passed.
- [x] (2026-04-24) Added the first Phase 7 results workspace slice: successful optimizer runs now render target signed exposure, funding decomposition, diagnostics, binding constraints, and rebalance preview rows from `:last-successful-run` without depending on scenario persistence.
- [x] (2026-04-24) Ran focused Phase 7 results validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 9 tests and 83 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Added the Phase 7 frontier selection slice: successful results now render an efficient-frontier panel, and clicking a frontier point dispatches the existing objective kind and objective parameter mutations so the draft is marked dirty and the user can rerun deliberately.
- [x] (2026-04-24) Ran focused frontier-selection validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 9 tests and 87 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Added the Phase 7 infeasible/binding-highlight slice: infeasible worker results now show a structured banner with failing constraint codes and affected controls, max-asset-weight and target-return controls can render invalid warning state, and target exposure rows with binding constraints are marked and visually highlighted.
- [x] (2026-04-24) Ran focused infeasible/highlight validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 10 tests and 94 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Completed the Phase 7 frontier interaction slice by making frontier points draggable; click, drag-start, and drag-enter all dispatch the same target objective mutations so dragging across points updates the draft target deliberately.
- [x] (2026-04-24) Ran focused frontier drag validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.view-test`; 10 tests and 97 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24) Added the first Phase 8 scenario-persistence slice: a solved optimizer run can be saved as a local scenario record, the address-scoped scenario index is updated through the existing IndexedDB persistence boundary, the mutable draft is marked clean, and the workspace save action is disabled until a solved run exists.
- [x] (2026-04-24) Ran focused scenario-save validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.scenario-records-test --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test --test=hyperopen.views.portfolio.optimize.view-test`; 66 tests and 390 assertions passed with zero failures and zero errors.
- [x] (2026-04-24 18:56Z) Added the next Phase 8 read-side persistence slice: optimizer routes now enqueue scenario index and scenario record loads, IndexedDB-backed load effects hydrate the scenario board and saved scenario workspace, and `/portfolio/optimize` renders saved scenario rows with lifecycle status plus objective, return-model, risk-model, expected return, and volatility summaries.
- [x] (2026-04-24 18:56Z) Ran focused route/load/board validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.views.portfolio.optimize.view-test --test=hyperopen.runtime.wiring-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.effect-adapters.facade-contract-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test --test=hyperopen.startup.route-refresh-test --test=hyperopen.runtime.action-adapters.navigation-test`; 91 tests and 490 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24 18:56Z) Added and ran focused browser coverage with `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "portfolio optimizer scenario board renders"`; 1 test passed and verified the `/portfolio/optimize` board route and navigation into the setup workspace.
- [x] (2026-04-24 19:00Z) Split the new board view assertions into `test/hyperopen/views/portfolio/optimize/index_view_test.cljs` after the namespace-size guard caught `view_test.cljs` growing past its limit.
- [x] (2026-04-24 19:00Z) Ran the required gates after the Phase 8 read-side slice: `npm run check`, `npm test`, `npm run test:websocket`, the focused Playwright board regression, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3521 tests and 19108 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions.
- [x] (2026-04-24 20:39Z) Added the Phase 8 duplicate/archive scenario slice: saved scenarios can now be archived in place, duplicated under a new id, and mutated through address-scoped index updates; the scenario board exposes row actions that stop row-navigation bubbling before dispatching archive/duplicate actions.
- [x] (2026-04-24 20:39Z) Split optimizer scenario persistence effects into `portfolio_optimizer_scenarios.cljs` and scenario state transitions into `portfolio_optimizer_scenario_state.cljs` after the namespace-size gate caught the expanded effect adapter; reran focused duplicate/archive validation with 69 tests and 431 assertions passing.
- [x] (2026-04-24 20:39Z) Ran required validation after duplicate/archive: `npm run check`, `npm test`, `npm run test:websocket`, focused Playwright board regression, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3527 tests and 19171 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions.
- [x] (2026-04-24 20:55Z) Correct Phase 6 scope gap: the setup workspace is not complete until it supports manual one-by-one universe construction. Add an asset search/select control that reuses existing asset-selector market state, add/remove draft instruments, preserves spot/perp identity, prevents duplicate instruments, cleans removed-instrument overrides, marks the draft dirty, and forces the existing history-readiness path to block until newly added assets have history.
- [x] (2026-04-24 21:16Z) Implemented the manual universe-builder slice: setup now has a dedicated universe panel namespace with search, add, and remove controls backed by the existing asset-selector market catalog; actions are runtime-registered, contract-checked, duplicate-safe, dirty-state aware, and clean dependent allowlist/blocklist/held-lock/override/perp-cap state on removal.
- [x] (2026-04-24 21:16Z) Ran focused manual-universe validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.views.portfolio.optimize.view-test --test=hyperopen.views.portfolio.optimize.universe-panel-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.portfolio.optimizer.defaults-test`; 50 tests and 390 assertions passed with zero failures and zero errors. Split the new UI coverage into `test/hyperopen/views/portfolio/optimize/universe_panel_test.cljs` after namespace-size lint caught the existing view test growing too large.
- [x] (2026-04-24 21:16Z) Added focused browser coverage for manual add/remove with `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "manual universe builder"`; 1 test passed and verified search, add, dirty-state, duplicate hiding, stale history copy, and remove/re-add behavior.
- [x] (2026-04-24 21:26Z) Addressed reviewer-found stale-history bug in the manual builder: readiness now blocks partial eligible-universe requests and in-flight history reloads, so adding `perp:ETH` after loading BTC-only history cannot run against a silently truncated universe. Added pure setup-readiness coverage plus view/browser assertions for the exact stale retained-history case.
- [x] (2026-04-24 22:04Z) Added the first Phase 9 execution foundation: a pure execution-plan builder now classifies rebalance preview rows into executable perp intents, blocked rows, and skipped tolerance rows; the results surface exposes a review-execution action and a read-only execution modal that clearly separates ready and blocked rows before real submit wiring is enabled.
- [x] (2026-04-24 22:04Z) Ran focused execution-modal validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.execution-test --test=hyperopen.portfolio.optimizer.actions-test --test=hyperopen.views.portfolio.optimize.execution-modal-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.portfolio.optimizer.defaults-test`; 45 tests and 312 assertions passed.
- [x] (2026-04-24 22:04Z) Added browser/network QA coverage for optimizer history loading. The probe verifies that adding an asset to the universe sends zero history requests, and clicking Load History for the newly added perp sends exactly one `candleSnapshot` and one `fundingHistory` request for that coin.
- [x] (2026-04-24 23:16Z) Added the Phase 9 execution-submit foundation: the confirm action now dispatches a dedicated optimizer execution effect, ready perp rows are converted into normalized market-order requests through the existing order-command builder, rows submit sequentially through the current trading submit boundary, execution ledgers record submitted/failed/blocked outcomes, active-scenario lifecycle moves to `:executed` or `:partially-executed` when appropriate, and account/order-history refresh is dispatched after successful mutation rows.
- [x] (2026-04-24 23:16Z) Ran focused execution-submit validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.execution-test --test=hyperopen.portfolio.optimizer.execution-actions-test --test=hyperopen.views.portfolio.optimize.execution-modal-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-execution-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.runtime.effect-adapters.facade-contract-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test --test=hyperopen.portfolio.optimizer.defaults-test`; 63 tests and 423 assertions passed with zero failures and zero warnings.
- [x] (2026-04-24 23:19Z) Ran required validation after the Phase 9 execution-submit slice: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3548 tests and 19266 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Playwright covered the scenario board, setup model layers, and manual universe builder regressions.
- [x] (2026-04-24 23:57Z) Added the Phase 9 execution persistence slice: execution ledgers now append to saved scenario records, saved scenario/config lifecycle state advances with the ledger status, scenario index summaries are refreshed through the existing IndexedDB persistence boundary, and persistence errors are surfaced on optimizer execution state without rolling back already-submitted rows.
- [x] (2026-04-24 23:57Z) Ran focused execution-persistence validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.scenario-records-test --test=hyperopen.portfolio.optimizer.application.execution-test --test=hyperopen.portfolio.optimizer.execution-actions-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-execution-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.runtime.effect-adapters.facade-contract-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test --test=hyperopen.views.portfolio.optimize.execution-modal-test`; 67 tests and 450 assertions passed with zero failures and zero warnings.
- [x] (2026-04-25 00:00Z) Ran required validation after the execution-persistence slice: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3550 tests and 19284 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Playwright covered the scenario board, setup model layers, and manual universe builder regressions.
- [x] (2026-04-25 00:08Z) Added the first Phase 10 tracking foundation: pure tracking snapshots now compute target-versus-current weight drift, `:weight-drift-rms`, `:distance-to-target`, max absolute drift, predicted return, and explicit not-trackable warnings; an on-demand tracking refresh action/effect persists snapshots through the optimizer tracking IndexedDB record for executed or partially executed scenarios.
- [x] (2026-04-25 00:08Z) Ran focused tracking-foundation validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.application.tracking-test --test=hyperopen.portfolio.optimizer.tracking-actions-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-tracking-test --test=hyperopen.portfolio.optimizer.defaults-test --test=hyperopen.runtime.action-adapters-test --test=hyperopen.runtime.effect-adapters.facade-contract-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-test --test=hyperopen.runtime.wiring-test --test=hyperopen.schema.contracts.action-args-test --test=hyperopen.schema.contracts.effect-args-test`; 56 tests and 411 assertions passed with zero failures and zero warnings.
- [x] (2026-04-25 00:11Z) Ran required validation after the tracking-foundation slice: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3556 tests and 19317 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Playwright covered the scenario board, setup model layers, and manual universe builder regressions.
- [x] (2026-04-25 00:51Z) Extended Phase 10 tracking into the routed workspace: saved scenario loads now hydrate the persisted tracking record when present, and executed/partially executed scenarios render a tracking panel with refresh, weight-drift RMS, max drift, predicted/realized return, and per-instrument current-vs-target drift rows.
- [x] (2026-04-25 00:51Z) Ran focused tracking UI/load validation with `npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.views.portfolio.optimize.tracking-panel-test --test=hyperopen.runtime.effect-adapters.portfolio-optimizer-tracking-test`; 4 tests and 16 assertions passed with zero failures and zero warnings, including stale-tracking cleanup when loading a scenario with no tracking record.
- [x] (2026-04-25 01:13Z) Ran required validation after the tracking UI/load slice and Playwright route-readiness hardening: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, and `npm run browser:cleanup`; all passed. Full `npm test` covered 3559 tests and 19327 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Playwright covered the scenario board, setup model layers, and manual universe builder regressions.
- [x] (2026-04-25 02:32Z) Started Phase 11 hardening with deterministic coverage for persisted scenario reload and worker performance. Added a Playwright regression that seeds optimizer IndexedDB records, verifies a persisted partially executed scenario hydrates results and tracking, reloads the browser route, and verifies the same panels still render. Added a worker runaway-budget test for 20-, 40-, and 60-instrument synthetic universes through the real worker payload path.
- [x] (2026-04-25 02:32Z) Ran focused Phase 11 validation with `npm run test:runner:generate && npx shadow-cljs --force-spawn compile test && node out/test.js --test=hyperopen.portfolio.optimizer.worker-test` and `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "persisted scenario hydrates"`; worker coverage passed with 3 tests and 18 assertions, and the focused Playwright reload regression passed.
- [x] (2026-04-25 02:35Z) Ran required validation after the first Phase 11 hardening slice: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, `npm run browser:cleanup`, and `git diff --check`; all passed. Full `npm test` covered 3560 tests and 19336 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Focused Playwright covered scenario board, setup model layers, manual universe builder, and persisted scenario tracking reload.
- [x] (2026-04-25 02:37Z) Added Phase 11 Spectate Mode browser coverage for optimizer execution safety. The regression seeds the same persisted scenario, enables Spectate Mode in the live store, opens the execution modal, and verifies the read-only message plus disabled confirm button. Focused validation with `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "read-only in Spectate Mode"` passed.
- [x] (2026-04-25 02:43Z) Added Phase 11 execution failure recovery visibility. The execution modal now renders the latest execution ledger attempt, including failed row status and row-level error messages, so a user can inspect why an execution failed before retrying. Added view coverage that failed before the change, plus a Playwright regression for failed attempt recovery details in the routed optimizer workspace.
- [x] (2026-04-25 02:45Z) Ran required validation after the execution failure recovery slice: `npm run check`, `npm test`, `npm run test:websocket`, focused portfolio optimizer Playwright regressions, `npm run browser:cleanup`, and `git diff --check`; all passed. Full `npm test` covered 3561 tests and 19341 assertions. `npm run test:websocket` covered 461 tests and 2798 assertions. Focused Playwright covered six optimizer regressions, including persisted tracking reload, Spectate Mode execution blocking, and failed execution attempt recovery details.
- [x] (2026-04-25 02:47Z) Added routed stale-result retention coverage for Phase 11. The Playwright regression loads a persisted scenario, forces an in-flight rerun state, and verifies the last successful results surface remains visible with the explicit "Retaining last successful result while rerunning" copy and disabled run button. Focused validation with `npx playwright test tools/playwright/test/portfolio-regressions.spec.mjs --grep "rerun keeps"` passed.
...
- [x] Implement setup/results UI, execution path, and tracking flow.
- [x] Complete Phase 11 QA, performance, and hardening gates.
...
- Observation: startup route refresh should enqueue optimizer route-load actions, while in-action navigation should call the optimizer route loader directly and return effects.
  Evidence: `src/hyperopen/startup/route_refresh.cljs` follows the existing app-level pattern of returning `[:actions/load-...-route]` on refresh, but `src/hyperopen/runtime/action_adapters/navigation.cljs` is already inside the action interpreter and must return `:effects/load-portfolio-optimizer-scenario-index` or `:effects/load-portfolio-optimizer-scenario` directly.
...
The universe panel has two required paths. First, it may seed from current holdings for fast portfolio-derived scenarios. Second, it must support manual one-by-one universe construction for arbitrary scenarios. The manual builder should reuse `[:asset-selector :markets]` and `[:asset-selector :market-by-key]` rather than creating a second market catalog; search results must preserve `:key`, `:coin`, `:market-type`, `:dex`, and spot/perp identity when inserting into `[:portfolio :optimizer :draft :universe]`. Adding an already-present instrument is a no-op. Removing an instrument must also remove it from allowlist, blocklist, held-position locks, per-asset overrides, and per-perp caps. Every add/remove marks the draft dirty, and the existing setup-readiness/history path must require history reload for newly added assets before a run becomes enabled.
...
- 2026-04-23 / Codex: Added explicit optimizer default state and replaced the setup placeholder with separate model-layer and constraints panels, preserving Black-Litterman as a return-model option.
- 2026-04-23 / Codex: Added runtime-registered setup draft mutation actions and clickable model-layer controls, with browser coverage for objective, return-model, and risk-model changes.
- 2026-04-23 / Codex: Added editable setup controls for global constraints and browser coverage for long-only and max-asset-weight draft updates.
- 2026-04-23 / Codex: Added current-holdings universe seeding through the runtime registry and surfaced it as the first setup universe control.
- 2026-04-24 / Codex: Added setup route run-launch wiring from draft state to the existing optimizer worker effect, including disabled run control behavior while the draft universe is empty.
- 2026-04-24 / Codex: Added setup readiness gating from the assembled optimizer request so missing history blocks run launch and surfaces request-builder warnings in the setup right rail.
- 2026-04-24 / Codex: Corrected setup-to-engine constraint normalization so draft controls feed the solver with the expected gross, net, allowlist, lock, perp-cap, and dust-threshold contracts.
- 2026-04-24 / Codex: Corrected optimizer worker request normalization so postMessage-decoded instrument-id maps keep string keys before engine execution.
- 2026-04-24 / Codex: Added setup controls for objective target values and execution assumptions, including request normalization for fallback slippage.
- 2026-04-24 / Codex: Added row-level setup controls for allowlist/blocklist membership, per-asset max weights, held-position locks, and per-perp caps.
- 2026-04-24 / Codex: Added setup workspace draft/run status surfaces, including failed-run detail and last-successful-run retention messaging.
- 2026-04-24 / Codex: Added setup history-load action/effect wiring and UI state so arbitrary-universe candles and perp funding can be fetched into optimizer-owned state before launching a run.
- 2026-04-24 / Codex: Added a results panel that reads the existing worker result contract directly, preserving signed current-vs-target exposure, funding decomposition, diagnostics, binding constraints, and blocked rebalance rows as first-class visible state.
- 2026-04-24 / Codex: Added a first efficient-frontier UI component with point-selection actions wired through the existing draft objective mutation path; drag-to-select remains a later refinement.
- 2026-04-24 / Codex: Added structured infeasible-run UX and binding row/control highlighting for the current results workspace; full drag selection and persisted scenario stale banners remain open.
- 2026-04-24 / Codex: Added drag selection to the frontier component by making points draggable and wiring drag-start/drag-enter through the existing draft objective action sequence.
- 2026-04-24 / Codex: Added save-current-solved-scenario persistence through the existing keyed IndexedDB wrapper. Scenario load from `/portfolio/optimize/:scenario-id`, the saved-scenario board, archive/duplicate actions, and stale-result banners remain open Phase 8 work.
- 2026-04-24 / Codex: Added scenario duplicate/archive persistence and board actions. Scenario stale-result banners remain the main open Phase 8 workspace item before rebalance execution work starts.
