...
The first release should stay intentionally small. It should ship one real security-adjacent preference that Hyperopen already supports technically, plus one low-effort alert preference, then define how later phases can add confirmations, transaction-delay controls, order-book animation, and chart markers without collapsing into a misleading clone of the full Hyperliquid dropdown.
...
- [ ] Convert this approved spec into implementation tickets and begin phase-1 execution.
...
- Decision: ship only two real controls in phase 1: `Remember trading session on this device` and `Show fill alerts in app`.
  Rationale: those are the only settings in the screenshot that map cleanly to existing Hyperopen behavior without inventing unsupported account semantics or new subsystems.
  Date/Author: 2026-03-19 / Codex
...
- Decision: defer transaction delay protection, account abstraction controls, and confirmation-skip toggles to later phases.
  Rationale: those controls change trading-risk semantics, require stronger copy and confirmation flows, and in some cases require signed platform actions rather than simple local preference writes.
  Date/Author: 2026-03-19 / Codex

- Decision: hide unsupported screenshot items in phase 1 instead of showing a long disabled list.
  Rationale: a short truthful surface is better than a parity-heavy panel full of non-functional controls. If product wants roadmap signaling later, use one brief non-interactive note rather than fake switches.
  Date/Author: 2026-03-19 / Codex
...
This planning pass produced a concrete, implementation-ready product spec instead of a generic wishlist. The spec narrows the Hyperliquid screenshot into a trustworthy Hyperopen surface: a real gear-driven settings entry point, a phase-1 scope tied to existing runtime surfaces, a safer naming scheme, explicit deferrals for account-mode controls, and a browser-QA contract that matches the governed UI workflow.

The plan reduces overall complexity. Instead of trying to model every screenshot checkbox as a local preference, it separates local UI toggles from account-level trading semantics, uses the existing wallet storage-mode authority for the security-adjacent preference, and makes later work explicit for confirmations, delay protection, chart markers, and account-mode mutation flows. No production code has been written yet; the remaining work is implementation and governed UI validation.
...
This product spec deliberately translates the screenshot into a smaller Hyperopen-specific surface. The phase-1 goal is not visual parity. The phase-1 goal is a trustworthy settings surface that a user can actually rely on.
...
Phase 1 should ship the settings shell and exactly two real preferences.
...
Phase 1 should not show account-mode rows, sound rows, chart rows, or warning rows. If product wants a third local setting after the first implementation lands cleanly, phase 1 should still remain closed and that setting should start a follow-up phase rather than bloating the first ship.

### V1.5

The next bounded phase can expand the panel with adjacent-surface preferences that do not require a full product rethink.

- `Animate order book`, default `ON`, implemented as a local UI preference that gates the current depth-bar transitions.
- `Confirm new orders`, default `ON`, only after Hyperopen first adds a real default-on pre-submit confirmation step for new orders.
- `Confirm position close`, default `ON`, only after Hyperopen first adds a real default-on confirmation step for close-position actions.
- `Expire delayed orders after 15 seconds`, default `ON`, as the positive-label version of Hyperliquid’s transaction delay protection. Turning it `OFF` should require stronger risk copy and an explicit confirm action.
- `Show fill markers on chart`, likely default `OFF`, after the chart marker path is wired to generic buy and sell fills.
- `Verbose error details`, default `OFF`, only if the scope is limited to user-safe diagnostics or advanced support surfaces rather than raw backend payload dumps.

### Later

Later work may include true browser background notifications with a permission flow, fill sounds, a product-wide sensitive-value or PNL hiding mode, a centralized warning taxonomy, and explicit account-mode management. Those are real features, not cosmetic checkboxes.
...
This plan does not cover layout customization, `Return to Default Layout`, spot dusting preferences, order-book click-size behavior, or any long disabled list built only to imitate the screenshot. This plan also does not claim to solve the repository’s browser-secret-storage risk beyond exposing the existing persistence choice more clearly. Stronger key-protection work remains separate.

## Plan of Work

First, add real settings-surface state to the header UI. The minimum state expansion is `:settings-open?` under `[:header-ui ...]`, plus any small confirmation-state shape needed for the trading-session persistence confirm step. The obvious wiring points are `/hyperopen/src/hyperopen/state/app_defaults.cljs`, `/hyperopen/src/hyperopen/header/actions.cljs`, `/hyperopen/src/hyperopen/app/actions.cljs`, `/hyperopen/src/hyperopen/runtime/collaborators.cljs`, `/hyperopen/src/hyperopen/core/public_actions.cljs`, `/hyperopen/src/hyperopen/schema/contracts.cljs`, and `/hyperopen/src/hyperopen/schema/runtime_registration_catalog.cljs`.

Next, update `/hyperopen/src/hyperopen/views/header_view.cljs` so the existing gear becomes the universal trigger for the new surface. The trigger must expose `aria-haspopup="dialog"` and `aria-expanded`, the settings entry point must stay available at all four governed review widths, and open or close transitions must happen before any persistence or trading-state effects.

Then add a bounded trading-preference record for local settings that truly belong to Hyperopen. The recommended shape is a new top-level subtree such as `:trading-settings`, restored during startup through a dedicated helper and persisted in one versioned `localStorage` record such as `hyperopen:trading-settings:v1`. Phase 1 needs only `:fill-alerts-enabled?`, but the versioned record can safely grow later to include `:animate-orderbook?`, `:confirm-new-orders?`, `:confirm-position-close?`, `:transaction-delay-protection?`, and `:verbose-errors-enabled?`. Do not duplicate trading-session persistence here; `[:wallet :agent :storage-mode]` remains the single source of truth for that setting.

After that, wire the two phase-1 preferences into the real runtime surfaces. `Remember trading session on this device` should read from the existing wallet storage mode and dispatch the existing storage-mode action only after the user accepts the confirmation step. `Show fill alerts in app` should gate toast emission in `/hyperopen/src/hyperopen/websocket/user_runtime/fills.cljs` without interfering with fill ingestion or account refresh behavior.

Finally, define follow-up issues before implementation expands beyond phase 1. The next obvious splits are order confirmation preferences, order-book animation, transaction delay protection, chart fill markers, and account-mode management. Those belong in linked `bd` tickets once implementation begins, not as implied backlog hidden inside the panel.
...
Start with `acceptance_test_writer` and `edge_case_test_writer` in parallel. Their scope is phase-1 only: the settings shell, the desktop and mobile presentation rules, the storage-mode confirmation path, startup restore behavior, and fill-toast suppression. After both proposals return, freeze one approved contract before any RED-phase test materialization begins.

Next run `tdd_test_writer` to materialize only the approved failing tests for the phase-1 contract. Verify the RED phase with the narrowest relevant commands before implementation starts.

Then run `ui_designer` before the main implementation pass. The UI-design task is to lock the final shape of the anchored desktop popover, the narrow-screen bottom sheet, the row grouping, the trust cues, the positive labels, and the confirmation-step copy for trading-session persistence. Any material UI decision it makes should be reflected back into this plan before code edits begin.
...
1. Keep this plan current while phase-1 implementation proceeds. Every stop point must update `Progress`, `Surprises & Discoveries`, `Decision Log`, and `Outcomes & Retrospective`.

2. Extend header UI state in:

    `/hyperopen/src/hyperopen/state/app_defaults.cljs`

   Add a settings-open flag and any minimal confirm-sheet state needed for the storage-mode change flow.
...
5. Add startup-restored trading settings in:
   ...
   Use one versioned `localStorage` record for tiny phase-1 and future phase-1.5 preferences. Follow `/hyperopen/docs/BROWSER_STORAGE.md`.

6. Wire the phase-1 runtime behavior in:
   ...
   `Show fill alerts in app` must only suppress toast creation. `Remember trading session on this device` must continue using the existing storage-mode authority and existing reset message.
...
7. Add or update tests in:
   ...
   Phase-1 tests should cover header open or close behavior, breakpoint entry-point presence where feasible, fill-toast suppression, settings restore, and the storage-mode confirmation path.
...
For future phase-1 implementation, acceptance is behavior, not just code edits.
...
The `Remember trading session on this device` row must mirror the real current wallet storage mode. Toggling it must not silently mutate storage. It must first show a short confirm step that explains the device scope and the need to re-enable trading. After confirmation, Hyperopen must reuse the existing storage-mode switch path and existing reset message rather than re-implementing that logic.

The `Show fill alerts in app` row must suppress new in-app fill toasts when `OFF` and preserve the current fill-toast behavior when `ON`. Fill ingestion, order refresh, and account-surface refresh behavior must remain unchanged.

Phase-1 acceptance also requires omission of unsupported high-stakes controls. The panel must not show active controls for account abstraction mode changes, transaction delay protection, or sound notifications in the first release.
...
This spec is safe to re-read and revise. The planned phase-1 code changes are ordinary tracked-file edits and startup preference writes. Re-running the restore logic should remain idempotent because the local preference record is small, versioned, and deterministic. If a later phase expands the preference schema, increment the record version explicitly and keep fallback behavior temporary.

If implementation accidentally drifts into unsupported parity work, stop and split that work into a new `bd` issue rather than letting the phase-1 panel accumulate inert switches. If a storage-mode change path risks invalidating a live trading session unexpectedly, prefer preserving the current wallet state and surface a clear retry path rather than force-resetting silently.
...
Phase-1 implementation should end with these stable state shapes and ownership boundaries:

In `/hyperopen/src/hyperopen/state/app_defaults.cljs`, `:header-ui` should grow to include:
...
Future phases may extend that subtree with:

    {:animate-orderbook? true
     :confirm-new-orders? true
     :confirm-position-close? true
     :transaction-delay-protection? true
     :verbose-errors-enabled? false}

Do not add trading-session persistence to that subtree. The authoritative interface for that preference remains the existing wallet agent storage mode under:

    [:wallet :agent :storage-mode]

No new library is required for phase 1. The implementation should stay within the existing Hyperopen view, header action, wallet, startup, websocket, and test modules named throughout this plan.
