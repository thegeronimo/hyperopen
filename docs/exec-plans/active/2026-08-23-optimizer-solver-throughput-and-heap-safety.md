...
### Found while doing this work, filed rather than fixed

- [ ] `normalize-black-litterman-view-weights` is non-total on the highest-frequency inbound path. `normalize-worker-boundary` runs on every message from the worker (`run_bridge.cljs:88-96` -> `worker_client.cljs:21-27`), and a payload whose `[:return-model :views]` is a scalar rather than a sequence throws inside the message listener — `{:return-model {:views 0.4}}` throws, `{:views "abc"}` silently becomes `[\a \b \c]`. Pre-existing and unchanged by the fusion in Milestone 9, but it is a latent crash on a hot path.
- [ ] `closed_form_support/within-bounds?` does not check every bound. It `map`s over `weights`, `lower-bounds` and `upper-bounds` together, and `map` with three collections stops at the shortest, so a bounds vector shorter than the weight vector leaves the tail unchecked. `closed-form-problem` currently papers over this on the QP-shaped keys, so it is latent rather than live — but the safety argument for any closed-form change leans on this function.
- [ ] `risk_ledoit_wolf_test.cljs` asserts its bit-parity with plain `=`, which reports two NaNs as different and `0.0` and `-0.0` as the same. Milestone 8 added `test/hyperopen/portfolio/optimizer/bit_parity.cljs` for exactly this; the Ledoit-Wolf suite should move onto it.
- [ ] The `osqp` wrapper drops `:eps_prim_inf` — it passes `:eps_dual_inf` twice into `_create_settings` — and does not expose `:adaptive_rho_interval` at all. Both are recorded in a comment on `osqp/settings`. The first is currently harmless because both default to 1e-4 and we set neither; the second is why solve counts and occasionally solutions drift between identical runs. Fixable in the vendored copy if either is ever wanted, at the cost of making the patch more than a leak fix.
