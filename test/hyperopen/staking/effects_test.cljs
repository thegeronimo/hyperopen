...
(defn- address-fetch-case
  [{:keys [effect-fn request-key begin-key success-key error-key result success-path]}]
  (let [store (atom {:router {:path "/trade"}
    ...
    (-> (effect-fn
         (assoc {:store store
                ...
                begin-key
                (fn [state]
                  (assoc-in state [:staking :loading success-path] true))
                success-key
                (fn [state payload]
                  (-> state
                      (assoc-in [:staking :loading success-path] false)
                      (assoc-in [:staking :success success-path] payload)))
                error-key
                (fn [state err]
                  (assoc-in state [:staking :error success-path] err))))
        (.then (fn [_]
                 (is (= [[owner-address {:source :coverage-test}]]
                        @request-calls))
                 (is (= result
                        (get-in @store [:staking :success success-path])))
                 (is (= false
                        (get-in @store [:staking :loading success-path]))))))))

(deftest address-scoped-staking-fetch-effects-resolve-wallet-address-and-strip-route-gate-test
  (async done
    (let [cases [{:effect-fn effects/api-fetch-staking-delegator-summary!
                 ...
                 {:effect-fn effects/api-fetch-staking-delegations!
                  ...
                  :result [{:validator "0xvalidator"}]
                  :success-path :delegations}
                 {:effect-fn effects/api-fetch-staking-rewards!
                  ...
                  :result [{:amount "1.5"}]
                  :success-path :rewards}
                 {:effect-fn effects/api-fetch-staking-history!
                 ...
                 {:effect-fn effects/api-fetch-staking-spot-state!
                  ...
                  :result {:balances []}
                  :success-path :spot-state}]]
      (-> (reduce (fn [promise case]
                    (.then promise #(address-fetch-case case)))
...
(deftest api-submit-staking-withdraw-success-clears-withdraw-input-and-refreshes-test
  (async done
    (let [store (atom {:wallet {:address owner-address}
      ...
      (-> (effects/api-submit-staking-withdraw!
           {:store store
            :request {:kind :withdraw
                      :action {:type "cWithdraw"
                               :wei 40000000}}
            :submit-c-withdraw! (fn [store* address action]
                                  (swap! submit-calls conj [store* address action])
                                  (js/Promise.resolve {:status "ok"}))
            :show-toast! (fn [_store kind message]
                           (swap! toasts conj [kind message]))
          ...
          (.then (fn [resp]
                   (is (= {:status "ok"} resp))
                   (is (= [[owner-address {:type "cWithdraw"
                                           :wei 40000000}]]
                          (mapv (fn [[_store address action]]
                                  [address action])
                                @submit-calls)))
...
(deftest api-submit-staking-delegate-rejects-read-only-account-context-test
  (let [store (atom {:wallet {:address owner-address}
    ...
    (is (= "Spectate Mode is read-only. Stop Spectate Mode to place trades or move funds."
           (get-in @store [:staking-ui :form-error])))
    (is (= [[:error "Spectate Mode is read-only. Stop Spectate Mode to place trades or move funds."]]
           @toasts))))

(deftest api-submit-staking-withdraw-rejects-invalid-action-payload-test
  (let [store (atom {:wallet {:address owner-address}
                     :staking-ui {:submitting {:withdraw? true}
                                  :form-error nil}})
        submit-calls (atom 0)
...
(deftest api-submit-staking-delegate-runtime-rejection-uses-fallback-message-test
  (async done
    (let [store (atom {:wallet {:address owner-address}
                       :staking-ui {:submitting {:delegate? true}
                                    :form-error nil}})
      ...
      (-> (effects/api-submit-staking-delegate!
          ...
          (.then (fn [_]
                   (is (= false (get-in @store [:staking-ui :submitting :delegate?])))
                   (is (= "Stake failed: Unknown runtime error"
                          (get-in @store [:staking-ui :form-error])))
                   (is (= [[:error "Stake failed: Unknown runtime error"]]
                          @toasts))
                   (done)))
          (.catch (fn [err]
