...
(deftest header-renders-trading-settings-shell-when-open-test
  (let [view (header-view/header-view {:wallet {:connected? false
    ...
    (is (contains? panel-classes "w-[368px]"))
    (is (contains? panel-classes "max-h-[min(calc(100vh-5.5rem),46rem)]"))
    (is (contains? panel-classes "rounded-[16px]"))
    (is (contains? panel-classes "bg-[#132026]"))
    (is (contains? panel-classes "overflow-visible"))
    (is (contains? sheet-classes "max-h-[min(calc(100svh-1.5rem),44rem)]"))
