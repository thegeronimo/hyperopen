...
(deftest default-draft-preserves-v1-model-layer-contract-test
  (let [draft (defaults/default-draft)]
    ...
    (is (= 3.0 (get-in draft [:constraints :gross-max])))
    (is (not (contains? (:constraints draft) :net-min)))
    (is (= 1.5 (get-in draft [:constraints :net-max])))
    (is (= 0.25 (get-in draft [:constraints :max-asset-weight])))
