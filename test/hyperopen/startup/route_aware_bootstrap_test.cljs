...
(deftest bootstrap-account-data-keeps-optimizer-route-eager-even-with-performance-tab-default-test
  (let [address "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
        store (atom {:wallet {:address address}
...
(deftest start-critical-bootstrap-fetches-immediate-assets-on-optimizer-route-test
  (async done
...
(deftest schedule-deferred-bootstrap-uses-idle-scheduler-on-optimizer-route-test
  (let [store (atom {:router {:path "/portfolio/optimize"}
                     :portfolio-ui {:account-info-tab :performance-metrics}})
        startup-runtime-atom (atom {:deferred-scheduled? false})
        idle-calls (atom [])
